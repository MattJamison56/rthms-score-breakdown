  ...
  return (
    <div className="h-full bg-linear-to-br from-slate-900 via-emerald-950 to-slate-900 flex flex-col p-8 relative">
      <div className="text-center mb-8 mt-8">
        ...
        <h2 className="text-2xl font-bold text-white mb-2">Food & Dining</h2>
            ...
            <div className="flex gap-4 justify-center">
              {foodData.overlapping.slice(0, 3).map(cuisine => (
                <div key={cuisine} className="text-center">
                  <div className="w-20 h-20 bg-emerald-500/20 backdrop-blur-sm rounded-2xl flex items-center justify-center text-4xl mb-2 border-2 border-emerald-500/30">
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 border border-slate-700/50">
            ...
            <div className="flex flex-wrap gap-2">
              {person1.tags.food.slice(0, 3).map(food => (
                <span key={food} className="text-2xl">{getCuisineIcon(food)}</span>
              ))}
            </div>
          </div>
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 border border-slate-700/50">
